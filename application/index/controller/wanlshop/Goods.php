...
class Goods extends Wanlshop {
        ...
        if ($this->request->isPost()) {
            ...
            if ($params) {
                ...
                try {
                    ...
                    $this->model->market_price = min($params['market_price']);
                    ...
                    foreach ($params['sku'] as $key => $value) {
                        if($params['category_id']==105 or $params['category_id']==104){
                            $params['integral'][$key]=0;
                        }
                        ...
                        $sku[] = [
                            ...
                            'integral' => $params['integral'][$key],
                        ];
                    }
                    //if ($params["category_id"] == 105) {
                    // $fsucc = $ffail = '';
                    // foreach ($params["fenxiao_succ"] as $key => $value) {
                    //     $fsucc .= $value . ",";
                    // }
                    // Db::name("good_goods")->insert(["gid" => $this->model->id, "succ" => $fsucc, "join_num" => $params['join_num'], 'returnmax' => $params['returnmax']]);
    ...
    public function edit($ids = null) {
        ...
        $skuItem = model('app\index\model\wanlshop\GoodsSku')
                ...
                ->field('id,difference,price,market_price,stock,weigh,sn,sales,state,integral')
                ->select();
        if ($this->request->isPost()) {
            $params = $this->request->post("row/a");
            if ($params) {
                ...
                try {
                    ...
                    $spuItem = isset($params['spuItem']) ? $params['spuItem'] : $this->error(__('请填写销售信息-产品属性-产品规格'));
                    // 写入表单
//                                  $this->model->sharetitle = $params['share_title'];
//                    $this->model->shareimage = $params['shareimage'];
                    $data = $params;
        ...
        $giveInfo = Db::name("good_goods")->where(["gid" => $ids])->find();
        $fsucc = explode(',', $giveInfo["succ"]);
        $ffail = explode(',', $giveInfo["fail"]);


        $this->assign('succ', $fsucc);
        $this->assign('cc', $giveInfo);
