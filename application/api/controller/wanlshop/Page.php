...
class Page extends Api {
    ...
    public function goods($ids = null) {
        ...
        $list = model('app\api\model\wanlshop\Goods')
                ->where('id', 'in', $ids)
                ->field('id,image,title,price,shop_id,comment,praise,market_price,category_id')
                ->select();
        foreach ($list as $row) {
            $skuInfo = \think\Db::name("wanlshop_goods_sku")->where(["goods_id" => $row["id"]])->find();
