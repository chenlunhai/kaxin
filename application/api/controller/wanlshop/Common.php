...
class Common extends Api {
    ...
    public function myteam() {
        if (true) {
            ...
            $uinfo =Db::name("user")->where(["pid"=>$this->request->post('uid')])->select();
            foreach ($uinfo as $key => $value) {
                $tinfo=Db::name("c_team_total")->where(["uid"=>$value["id"]])->find();
                if($tinfo){
                    $uinfo[$key]["my_total"]=$tinfo["my_total"];
                }else{
                    $uinfo[$key]["my_total"]=0;
                }
            }
    ...
    public function share() {
        if (true) {
            ...
            if (!file_exists($filename) || 1) {
                ...
                imagecopyresampled($img1, $img2, 190, 430, 0, 0, 350, 350, 235, 235);
