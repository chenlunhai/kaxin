...
class Signin extends Api
{
	...
	public function getSignin()
	{
		...
		// 用户积分
		$data['user_score'] = $this->auth->score+$this->auth->integral;
                $data['user_score']= number_format($data['user_score'] ,2);
		// 已签到日期
		$data['list'] = $list;
		// 今日
		$data['date'] = $date;
	    ...
	    $successions++;
            ...
            $uino=Db::name("user")->where(["id"=>$this->auth->id])->find();
	    $score = isset($signdata['s' . $successions]) ? $signdata['s' . $successions] : $signdata['sn'];
		// 是否签到
		$data['signin'] = $signin;
		// 可获得积分
		$data['score'] = $score;
                $data['balance']=$uino["balance"];
		// 连续第几天
